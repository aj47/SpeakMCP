...
/**
 * ToolManager - Manages MCP tool filtering and enable/disable state
 *
 * Responsibilities:
 * - Track available tools from servers
 * - Manage disabled tools
 * - Filter tools by profile configuration
 * - Provide detailed tool lists
 */
export class ToolManager {
   ...
   */
  addServerTools(serverName: string, tools: Array<{ name: string; description?: string; inputSchema: any }>): void {
  ...
  /**
   * Get all available tools (filtered by runtime disabled servers and tools)
   */
  ...
  /**
   * Get detailed tool list with server and enabled status
   */
  ...
  /**
   * Apply disabled tools from profile config
   */
  applyDisabledTools(disabledTools?: string[]): void {

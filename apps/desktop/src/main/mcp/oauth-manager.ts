...
/**
 * OAuthManager handles OAuth authentication for MCP servers
 * Manages OAuth clients, token storage, and authentication flows
 */
export class OAuthManager {
   ...
   */
  async getValidToken(serverName: string, serverConfig: MCPServerConfig): Promise<string> {
    const oauthClient = await this.getOrCreateOAuthClient(serverName, serverConfig)
    return await oauthClient.getValidToken()
  }

  /**
   * Check if we have valid OAuth tokens for a server
   */
  async hasValidTokens(serverUrl: string): Promise<boolean> {
    return await oauthStorage.hasValidTokens(serverUrl)
  }
   ...
   */
  async handle401AndRetryWithOAuth(serverName: string, serverConfig: MCPServerConfig): Promise<StreamableHTTPClientTransport> {
    if (!serverConfig.url) {
  ...
  /**
   * Get OAuth client for a server (if exists)
   */
  getClient(serverName: string): OAuthClient | undefined {
    return this.oauthClients.get(serverName)
  }
}

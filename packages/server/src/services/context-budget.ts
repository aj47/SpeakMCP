...
// ============================================================================
// DEBUG UTILITIES (Stubs until debug module is ported)
// ============================================================================
...
async function fetchGroqContextWindow(model: string): Promise<number | undefined> {
  try {
    ...
    const data = await resp.json() as { data?: Array<{ id?: string; context_length?: number; max_context_tokens?: number; context_window?: number }> }
...
export async function getMaxContextTokens(providerId: string, model: string): Promise<number> {
  ...
  const override = cfg.mcpMaxContextTokensOverride as number | undefined
...
export function getProviderAndModel(): { providerId: string; model: string } {
  const config = configStore.get() as Record<string, unknown>
  const providerId = (config.mcpToolsProviderId as string) || "openai"
  let model = "gpt-4o-mini"
  if (providerId === "openai") {
    model = (config.mcpToolsOpenaiModel as string) || "gpt-4o-mini"
  } else if (providerId === "groq") {
    model = (config.mcpToolsGroqModel as string) || "llama-3.3-70b-versatile"
  } else if (providerId === "gemini") {
    model = (config.mcpToolsGeminiModel as string) || "gemini-1.5-flash-002"
  }
  return { providerId, model }
}
 ...
 * This is a stub that returns the input text - will be fully implemented when llm-fetch.ts is ported.
 *
 * @param content - The content to summarize
 * @param _sessionId - Optional session ID for cancellation support
 * @returns The summarized content (currently returns input as-is)
 */
export async function summarizeContent(content: string, _sessionId?: string): Promise<string> {
  // TODO: Implement actual LLM-based summarization when llm-fetch.ts is ported
  // For now, return the original content (compaction will be skipped due to length check)
  logLLM('[summarizeContent] Stub implementation - returning original content')
  return content
}

// ============================================================================
// SUMMARIZATION SERVICE STUB
// ============================================================================

// Stub for summarization service - will be fully implemented when summarization-service.ts is ported
...
const summarizationServiceStub = {
  getSummaries(_sessionId: string): AgentStepSummary[] {
    return []
  },
  getImportantSummaries(_sessionId: string): AgentStepSummary[] {
    return []
  }
}
...
function buildContextFromSummaries(sessionId: string): string | null {
  ...
  const summaries = summarizationServiceStub.getSummaries(sessionId)
  if (summaries.length === 0) return null

  // Get important summaries
  const important = summarizationServiceStub.getImportantSummaries(sessionId)
...
function parseToolNameFromContent(content: string): { toolName: string; resultContent: string } {
  ...
  // Using [\s\S]* instead of .* with 's' flag for multiline compatibility
  const match = content.match(/^\[([^\]]+)\]\s*(?:ERROR:\s*)?([\s\S]*)$/)
  if (match) {
...
/**
 * Construct a minimal system prompt for context budget management.
 * This is a stub - will be replaced when system-prompts.ts is ported.
 */
function constructMinimalSystemPrompt(
  _availableTools: Array<{ name: string; description?: string; inputSchema?: unknown }>,
  _isAgentMode: boolean,
  _relevantTools?: Array<{ name: string; description?: string; inputSchema?: unknown }>,
): string {
  return "You are a helpful AI assistant. Be concise and efficient."
}

// ============================================================================
// SHRINK OPTIONS AND RESULT TYPES
// ============================================================================

export interface ShrinkOptions {
  messages: LLMMessage[]
  availableTools?: Array<{ name: string; description?: string; inputSchema?: unknown }>
  relevantTools?: Array<{ name: string; description?: string; inputSchema?: unknown }>
  isAgentMode?: boolean
...
export async function shrinkMessagesForLLM(opts: ShrinkOptions): Promise<ShrinkResult> {
  ...
  const targetRatio = opts.targetRatio ?? ((config.mcpContextTargetRatio as number) ?? 0.7)
  const lastN = opts.lastNMessages ?? ((config.mcpContextLastNMessages as number) ?? 3)
  const summarizeThreshold = opts.summarizeCharThreshold ?? ((config.mcpContextSummarizeCharThreshold as number) ?? 2000)
  ...
  if (toolResultsSummarized && opts.sessionId) {
    ...
    if (progressSummary) {
      ...
      if (firstUserMsgIdx >= 0 && firstUserMsgIdx < messages.length - 1) {
        messages.splice(firstUserMsgIdx + 1, 0, { role: "assistant", content: progressSummary })
        tokens = estimateTokensFromMessages(messages)

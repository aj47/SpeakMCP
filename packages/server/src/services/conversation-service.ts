...
import { getConversationsFolder } from '../config'
import {
  Conversation,
  ConversationMessage,
...
export class ConversationService {
  ...
  private ensureConversationsFolder() {
    const conversationsFolder = getConversationsFolder()
  ...
  async loadConversationWithCompaction(conversationId: string, sessionId?: string): Promise<Conversation | null> {
    ...
    try {
      ...
      console.log(`[ConversationService] Failed to compact conversation ${conversationId}, returning original: ${error}`)
      return conversation
    }
  }
  ...
  async getConversationHistory(): Promise<ConversationHistoryItem[]> {
    try {
      ...
      console.log("[ConversationService] Error loading conversation history:", error)
      return []
    }
  }
  ...
  private async compactOnLoad(conversation: Conversation, sessionId?: string): Promise<Conversation> {
    ...
    console.log(`[ConversationService] compactOnLoad: compacting ${messagesToSummarize.length} messages for ${conversation.id}`)
    ...
    try {
      ...
      if (summaryContent === summarizationPrompt || summaryContent.length >= summarizationPrompt.length * 0.9) {
        console.log(`[ConversationService] compactOnLoad: summarization likely failed (output too similar to input), keeping original`)
        return conversation
      }
    } catch (error) {
      console.log(`[ConversationService] compactOnLoad: summarization failed, keeping original:`, error)
      return conversation
    }
    ...
    try {
      ...
      console.log(`[ConversationService] compactOnLoad: failed to persist, returning original:`, error)
      return conversation
    }

    console.log(`[ConversationService] compactOnLoad: compacted ${messagesToSummarize.length} messages into summary, new count: ${compactedConversation.messages.length}`)

...
import type { LLMToolCallResponse, MCPTool } from '../types'
import { diagnosticsService } from './diagnostics'
import { state, agentSessionStateManager, llmRequestAbortManager } from "./state"

/**
 * Build token usage object, only including it when at least one token field is present.
...
function convertMCPToolsToAISDKTools(mcpTools: MCPTool[]): ConvertedTools {
  ...
  for (const mcpTool of mcpTools) {
    ...
    if (nameMap.has(sanitizedName) && nameMap.get(sanitizedName) !== mcpTool.name) {
      ...
      console.log(`⚠️ Tool name collision detected: "${mcpTool.name}" and "${existingOriginal}" both sanitize to "${sanitizedName}"`)
      ...
      console.log(`   Disambiguated to: "${sanitizedName}"`)
    }
...
async function withRetry<T>(
  ...
  const maxRetries = options.maxRetries ?? (config.apiRetryCount as number) ?? 3
  const baseDelay = options.baseDelay ?? (config.apiRetryBaseDelay as number) ?? 1000
  const maxDelay = options.maxDelay ?? (config.apiRetryMaxDelay as number) ?? 30000
...
/**
 * Chat message type for message preparation
 */
...
/**
 * Prepare chat messages for LLM calls
 */
    ...
    async () => {
      ...
      try {
        ...
        if (result.toolCalls && result.toolCalls.length > 0) {
          console.log("✅ AI SDK native tool calls received", {
        ...
        console.log("✅ AI SDK text response received", {
        ...
        if (jsonObject && (jsonObject.toolCalls || jsonObject.content)) {
          ...
          // Restore original tool names using nameMap if available
...
export async function streamLLMResponse(
  ...
  try {
    console.log("🚀 AI SDK streamText call", {
      provider: effectiveProviderId,
      messagesCount: messages.length,
      hasSystem: !!system,
    })
 ...
 * Wrapper for makeLLMCallWithFetch for backward compatibility
 */
export async function generateLLMResponse(
  ...
  return makeLLMCallWithFetch(messages, providerId, onRetryProgress, sessionId, tools)
}
  ...
  return withRetry(
    async () => {
      ...
      try {
        ...
        console.log("🚀 AI SDK text completion call", {
          provider: effectiveProviderId,
  ...
  return withRetry(
    async () => {
      ...
      try {
        ...
        console.log("🚀 AI SDK verification call", {
          provider: effectiveProviderId,
          messagesCount: messages.length,
          hasSystem: !!system,
        })
